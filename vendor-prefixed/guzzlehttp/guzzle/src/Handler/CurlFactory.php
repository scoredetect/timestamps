...
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Exception\ConnectException;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Exception\RequestException;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Promise as P;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Promise\FulfilledPromise;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Promise\PromiseInterface;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Psr7\LazyOpenStream;
use SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\TransferStats;
...
use SDCOM_Timestamps\Vendor_Prefixed\Psr\Http\Message\UriInterface;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \SDCOM_Timestamps\Vendor_Prefixed\GuzzleHttp\Psr7\Utils::streamFor($sink);
